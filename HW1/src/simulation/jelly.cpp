...
void Jelly::computeInternalForce() {
    ...
    auto springCount = this->getSpringNum();
    for (int i = 0; i < springCount;i++) {
        Spring &s = this->getSpring(i);
        Particle &particleA = this->getParticle(s.getSpringStartID());
        Particle &particleB = this->getParticle(s.getSpringEndID());

        Eigen::Vector3f positionA = particleA.getPosition(), positionB = particleB.getPosition();
        Eigen::Vector3f velocityA = particleA.getVelocity(), velocityB = particleB.getVelocity();

        Eigen::Vector3f spring_force =
            computeSpringForce(positionA, positionB, s.getSpringCoef(), s.getSpringRestLength());
        Eigen::Vector3f damper_force =
            computeDamperForce(positionA, positionB, velocityA, velocityB, s.getDamperCoef());

        Eigen::Vector3f totalForce = spring_force + damper_force;
...
Eigen::Vector3f Jelly::computeSpringForce(const Eigen::Vector3f &positionA, const Eigen::Vector3f &positionB,
    ...
    Eigen::Vector3f delta = positionA - positionB;
    Eigen::Vector3f spring_force = -springCoef * (delta.norm() - restLength) * delta / delta.norm();
...
Eigen::Vector3f Jelly::computeDamperForce(const Eigen::Vector3f &positionA, const Eigen::Vector3f &positionB,
    ...
    Eigen::Vector3f damper_force = -damperCoef * deltaV.dot(delta) * delta / (delta.squaredNorm());
